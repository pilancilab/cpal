...
def parse_args():
    ...
    parser.add_argument('--save_dir', type = str, default = "class_plots", help = "saving directory for plots.")
    parser.add_argument('--save_pickle', type = str, default = "class_accuracy", help = "saving directory for accuracy.")
...
def setup_seed(seed):
    ...
    torch.backends.cudnn.enable = False  # Disable cuDNN for reproducibility

def run_active_learning(args):
    setup_seed(args.seed)  # Set random seed for reproducibility
    ...
    return selected_indices, acc_train_list, acc_test_list, strategy, dataset, net, device
    

def plot_decision_boundary(net, dataset, selected_indices, strategy, device, save_dir="class_plots"):
...
def save_acc_pickle(acc_train_list, acc_test_list, strategy_name, save_dir):
    os.makedirs(save_dir, exist_ok=True)
    with open(os.path.join(save_dir, f'acc_{strategy_name}_test.pkl'), 'wb') as f:
        pickle.dump(acc_test_list, f)

    with open(os.path.join(save_dir, f'acc_{strategy_name}_train.pkl'), 'wb') as f:
        pickle.dump(acc_train_list, f)
    
def main():
    args = parse_args()
    selected_indices, acc_train_list, acc_test_list, strategy, dataset, net, device = run_active_learning(args)
    plot_decision_boundary(net, dataset, selected_indices, strategy, device, save_dir = args.save_dir)
    print(f"\nFinal selected indices: {selected_indices}")
    print(f"Accuracy train: {acc_train_list}")
    print(f"Accuracy test: {acc_test_list}")

    save_acc_pickle(acc_train_list, acc_test_list, args.strategy_name, save_dir=args.save_pickle)
